            ...
            <div class="block">
                ...
                <table>
                    <tr>
                        ...
                        <td class="w-30">
                            @if ($data->record)
                                {{ Helpers::getDivisionName($data->division_id) }}/AI/{{ Helpers::year($data->created_at) }}/{{ str_pad($data->record, 4, '0', STR_PAD_LEFT) }}
                            @else
                                Not Applicable
                            @endif
                        </td>
                        ...
                        <td class="w-80">
                            <div>
                                @if ($data->Reference_Recores1)
                                    @php
                                        $referenceIds = explode(',', $data->Reference_Recores1);
                                        $referenceValues = [];

                                        foreach ($referenceIds as $id) {
                                            // Find the record by ID
                                            $record = \App\Models\ActionItem::find($id); // Replace 'YourModel' with your actual model name

                                            if ($record) {
                                                // Get the division name and formatted record
                                                $divisionName = Helpers::getDivisionName($record->division_id);
                                                $formattedRecord = Helpers::recordFormat($record->record);
